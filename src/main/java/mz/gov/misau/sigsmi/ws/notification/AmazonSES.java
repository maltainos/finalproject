...
public class AmazonSES {
	...
	final String HTMLBODY = "<h1>Plaese verify your email addresses</h1> "
			+ "Thank you for registering with our SIGSSMI-MISAU" + "Here, registration confirmation click the link "
			+ "<a href='http://localhost:8080/verification-email-service/email-verification.html?token=$tokenValue'>"
			+ "Final step to complete your registration</a><br/><br>" + "Thank you! And we are waiting for you aside";
	...
	public void verifyEmail(UserDTO user) {
		...
		AmazonSimpleEmailService client = AmazonSimpleEmailServiceClientBuilder.standard()
				.withCredentials(new AWSStaticCredentialsProvider(credentials)).withRegion(Regions.AF_SOUTH_1).build();
		...
		SendEmailRequest request = new SendEmailRequest()
				...
				.withMessage(new Message()
						.withBody(new Body().withHtml(new Content().withCharset("UTF-8").withData(htmlStringWithToken))
